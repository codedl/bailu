...
void mainForm::handleSelectKey(int funcCode)
{
	...
	switch (tempCode)
	{
		...
		case KEY_FUNC_SYSTEM_TIME://设置时间
			{
				indexOfTime = 0;
				previndexOfTime = 0;
				//判断时间是否合法
				QStringList timeList = timeInput.split(":");
				int hour = timeList.at(0).toInt();
				int miniute = timeList.at(1).toInt();
				int second = timeList.at(2).toInt();
				if(0 <= hour && hour < 24 && 0 <= miniute && miniute < 60 && 0 <= second && second < 60)
				{
					printf("time:%s\n",timeInput.toStdString().c_str());
					if(!system(QString("date -s %1 ").arg(timeInput).toStdString().c_str()) && !system("hwclock -w")){
						strcpy(sysData.value, "time update");
					}
				}else{
						strcpy(sysData.value, "00:00:00");
				}
				drawInputParamter();
			}
			break;
...
//========================================================= System ============================================
		...
		case KEY_FUNC_SYSTEM_SERVICE: //系统服务
			printf("KEY_FUNC_SYSTEM_SERVICE handle\n");
			name = "Pass Word:";
			value = "******";
			unit = "";
			sysData.inputLength = 6;
			break;
		case KEY_FUNC_SYSTEM_TIME: //系统时间
			printf("KEY_FUNC_SYSTEM_TIME handle\n");
			name = "TIME:";
			value= "00:00:00";
			unit = "";
			indexOfTime = 0;
