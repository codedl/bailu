...
void mainForm::drawInputParamter(void)
{
	...
	if (!pix.isNull())
	{
		...
		p.setFont(fontWin2En);
		//draw name
		p.drawText(rcName, Qt::AlignLeft | Qt::AlignVCenter, QString(QObject::tr(sysData.name)).trimmed());
		//draw value
...
void mainForm::execNumberInput(QString value, bool isNumber)
{
	...
	else if(isTime())
	{
		...
		if (indexOfTime == 0 || previndexOfTime == 0)
		{
			inputValue = value;
		} else
		{
			if (indexOfTime > previndexOfTime) //显示字符的位置改变
			{
				inputValue += value;

				if (previndexOfTime == indexOfTime - 1)
				{
					prevInput = inputValue;
				}
			} else //显示字符的位置不改变
		...
		previndexOfTime = indexOfTime;
		timeInput = inputValue;
		for(int i=0; i<6-indexOfTime; i++)
			timeInput.append("0");
		timeInput.insert(2,":");
		timeInput.insert(5,":");		
		printf("inputvalue:%s\n",inputValue.toStdString().c_str()); 		
		printf("timeInput:%s\n",timeInput.toStdString().c_str()); 
		strcpy(sysData.value, timeInput.toStdString().c_str());
		strcpy(sysData.unit, "");
		return ;
...
//时间
bool mainForm::isTime(void)
{
	return nowSubKey != NULL && nowSubKey->funcCode == KEY_FUNC_SYSTEM_TIME;
}
