#ifndef SYSSTRUCT_H_
#define SYSSTRUCT_H_

#define FORMCREATETIME  0                     //0ms
#define FORMCLOSETIME   1000 * 500            //500ms

#define LISTENPORT      4060                  //监听端口
#define SOCKETBUFSIZE   1024 * 1024           //缓冲区大小

#define SCPIBUFSIZE     8192                  //SCPI缓冲区大小

#define LOGCOUNT        9                    //系统日志数目

#define DEVICETYPE		0						//设备类型 0：SG1060A， 1：SG1060B

#define SERIALNO        "00000000000000"              //序列号

#define MAXFREQINPUTLENGTH   10
#define MAXFREQMULINPUTLENGTH	3			//频率倍频系数输入最大长度
#define MAXAMPLINPUTLENGTH   5
#define MAXTIMEINPUTLENGTH   8
#define MAXPOINTINPUTLENGTH  5
#define MAXPLUSEINPUTLENGTH  12  //脉冲调制输入参数最大长度
#define MAXLFINPUTLENGTH     4   //低频输出幅度参数最大长度
#define MAXDRGFREQINPUTLENGTH	10	//DRG功能频率的最大长度
#define MAXDRGTIMEINPUTLENGTH	9	//DRG功能速率的最大长度

#define PASSWORD        203093                //系统服务口令
#define MINMACADDR      6785451               //最小MAC地址
#define MAXMACADDR      16777215              //最大MAC地址

#define MINGPIBADDR     1                     //最小GPIB地址
#define MAXGPIBADDR     31                    //最大GPIB地址

#define MAXINPUTLENGTH  7                    //最大输入长度
#define MAXFILECOUNT    1024                  //最大文件数量

#define MINFREQ         9e3                   //最小频率
#define MAXFREQ         6e9                  //最大频率		/* 限制最大频率时只要更改此处值即可 */
#define MINFREQOFFSET   -MAXFREQ               //最小频率偏置
#define MAXFREQOFFSET   MAXFREQ                //最大频率偏置

#define MAXAMPL         10.00	//当频率在500kHz ~ MAXFREQ时，幅度最大值为10dBm
#define MINAMPL         -120.00//-100.00
#define MAXAMPLFIRST		0.00	//当频率在9kHz ~ 500kHz时，幅度最大值为0dBm
#define MAXAMPLSECOND		5.00	//当频率在500kHz ~ 10MHz时，幅度最大值为5dBm

#define MAXAMPLOFFSET   120//100
#define MINAMPLOFFSET   -120.00  //-100.00

#define MAXEXTREFFREQ   300e6                 //最大 变频参考频率
#define MINEXTREFFREQ   25e6                 //最小 变频参考频率

#define MAXAMDEPTH 99.99
#define MINAMDEPTH 0.01


#define MAXLFFREQSINE  500e3		//LF频率中正弦波最大值
#define MAXLFFREQSQUARE  20e3		//LF频率中方波最大值
#define MAXLFFREQTRIANGLE  100e3	//LF频率中三角波最大值
#define MAXLFFREQSAWTOOTH  100e3	//LF频率中锯齿波最大值
#define MAXLFFREQSINC  100e3		//LF频率中辛格函数最大值
#define MINLFFREQ  0.1				//LF频率值最小值

#define MAXLFAMPL  4000    			//LF幅度最大值
#define MINLFAMPL  400     			//LF幅度最小值

#define MXADRGUPDOWNLIMIT 20e6		//DRG上下限最大差值

#define USBPOWERFREQMAX		MAXFREQ	//USB功率计最大频率
#define USBPOWERFREQMIN		MINFREQ	//USB功率计最小频率
#define USBPOWERAMPLOFFSETMAX	100	//USB功率计幅度偏移最大值
#define USBPOWERAMPLOFFSETMIN	-100//USB功率计幅度偏移最小值
#define USBPOWERADVCOUNTMAX	2048	//USB功率计幅度平均最大值
#define USBPOWERADVCOUNTMIN 2		//USB功率计幅度平均最小值

#define OPEN 	0x01
#define CLOSE 	0x00
#define LOCAL	0
#define REMOTE	1

#define UP 1
#define DOWN 0

#define CH1ANODESWITCH		1
#define CH1CATHODESWITCH	2
#define CH2ANODESWITCH		3
#define CH2CATHODESWITCH	4
#define	TRIGGERSWITCH		5


#define MINPLUSEPERIOD      200
#define MAXPLUSEPERIOD      160e9

#define MINPLUSEWIDTH       100
#define MAXPLUSEWIDTH       80e9

#define MINPLUSEDELAY       100
#define MAXPLUSEDELAY       ((160e9)-(100))

#define MINOSKINTERVAL      1
#define MAXOSKINTERVAL      260e3

#define MINPSKRATE         1
#define MAXPSKRATE         100e3

#define MINASKRATE         1
#define MAXASKRATE         10e6

#define MINFSKRATE         1
#define MAXFSKRATE         20e6

#define MAXDWELL		  5e3
#define MINDWELL		  20

const unsigned char logoBits[] = {};

enum incrsetTypeDef {itdFreq, itdAmptd};

//10MHz ref
enum ref10MHzDef {rmInternal, rmExternal};

//LF类型
enum lfOutputTypeDef {lotSine, lotSquare, lotTriangle, lotSawtooth, lotSinc};  //正弦波，方波，三角波，锯齿波，辛格函数

//DRG扫描方向
enum drgDireTypeDef {ddnormal, ddpositive, ddnegative};

//参考类型
enum extRefFrequencyTypeDef {erftInternal, erftExternal};

//连续性
enum sweepRepeatDef {srSingle, srConutinuos};

//方向
enum sweepDirectionDef {sdUp, sdDown};

//折返
enum sweepRetraceDef {srOn, srOff};

//扫描触发
enum sweepTriggerDef {stAuto, stExt, stKey};  //自动扫描，外部触发

//模式
enum sweepModeDef {smOff, smFreq, smAmpl, smFreqAmpl};

//类型
enum sweepTypeDef {stStep, stList};

//am内部函数波形
enum amInternalWaveTypeDef {aiwSine, aiwTriangle, aiwSquare, aiwSawtooth};
enum fmInternalWaveTypeDef {fiwSine, fiwTriangle, fiwSquare, fiwSawtooth};
enum pmInternalWaveTypeDef {piwSine, piwTriangle, piwSquare, piwSawtooth};

//fmpm demod
enum fmpmDemodTypeDef {fdtFM, fdtPM};
enum fmSourceTypeDef {fstInternal, fstExternal};
enum pmSourceTypeDef {pstInternal, pstExternal};

//digital demod
enum digitalDemodTypeDef {ddtNone, ddtAsk, ddtFsk, ddtPsk, ddtOsk};
enum digitalSourceTypeDef {dstInternal, dstExternal};

// mode
enum askModeDef {askm2, askm4, askm8};
enum fskModeDef {fskm2, fskm4, fskm8};
enum pskModeDef {pskmBpsk, pskmQpsk, pskmPI4Psk, pskm8};
enum oskStepDef {osks1, osks2, osks3, osks4};

//digital type SG10301A
enum digitalTypeDef {dt2ASK, dt4ASK, dt8ASK, dt2FSK, dt4FSK, dt8FSK, dt2PSK, dt4PSK, dt8PSK};

//digital type SG10301B
//enum digitalTypeDef {dtASK, dtMSK, dt2FSK, dt4FSK, dtBPSK, dtQPSK, dtOQPSK, dtPi2DBPSK, dtPi4QPSK, dtPi4DQPSK, dt8PSK, dt16QAM, dt32QAM, dt64QAM, dt128QAM};

//脉冲调制方式
enum pulseMethodDef{plmFreerun, plmKeytrig, plmExttrig};
//脉冲源
enum pulseSourceDef{psInternal, psExternal};
//脉冲调制外部触发信号极性
enum pulseExtPolarityDef{pepRising, pepFall};

//10MHz ref 类型
enum freqRefTypeDef {frtInterrior, frtExternal};

//日期时间格式
enum dateTimeStyle {dtsYMDHMS, dtsHMSYMD};

//波特率定义
enum baudRate {br1200, br2400, br4800, br9600, br19200};

//数据位定义
enum dataBits {db5, db6, db7, db8};

//停止位定义
enum stopBits {sb1, sb2};

//校验定义
enum checkMode {cmEven, cmOdd, cmNone, cmSpace};

//流控制定义
enum flowControl {fcXonXoff, fcHardware, fcNone};

//输入类型
enum inputType {itDouble, itDate, itTime, itIP, itPassword, itUnDefine};

//幅度单位定义
enum amplUnitDef {audBm, audBuV, aumV};

//频率单位定义
const static char* FREQUNIT[] = {"GHz", "MHz", "kHz", "Hz"};

//SCPI接口定义
enum scpiInterfaceType {sitNone, sitLan, sitUsb, sitCom};


//
enum drawTypeDef {dtFreq, dtAmpl, dtDwell, dtPoint};

//USB功率计平均模式
enum usbPowerAdvModeTypeDef{upamtOff, upamtAuto};
//USB功率计测量单位
enum usbPowerMeasUnitTypeDef{upmutdBm, upmutW};


static const char* SCPICMDERROR = "ERR";
static const char* SCPICMDUNSUPPORT = "N/A";
static unsigned char SCPIRESULTEOF[] = {0x0d, 0x0a};
static unsigned char SCPICMDEOF[] = {0x0d, 0x0a};

//标签汉化定义
struct labelTransDef
{
  const char* enName;
  const char* chName;
};

//rpg
static struct labelTransDef rpgTrans[] =
{
  {"rpg button", "脉冲旋钮：仪器参数设置方式之一，按当前参数的固定比例改变参数值设置；向左旋转参数值递减，右旋转递增"},
};

//arrow
static struct labelTransDef arrowTrans[] =
{
  {"up arrow", "方向键(上)：仪器参数设置方式之一，按固定步进方式增大当前参数值设置"},
  {"down arrow", "方向键(下)：仪器参数设置方式之一，按固定步进方式减小当前参数值设置"},
};

//cancel
static struct labelTransDef cancelTrans[] =
{
  {"cancel input", "取消：撤消参数输入和修改，对于程控模式时可切换回本地控制"},
};

//backspace
static struct labelTransDef bkspaceTrans[] =
{
  {"backSpace", "退格：逐步撤消参数输入"},
};

//enter
static struct labelTransDef enterTrans[] =
{
  {"input enter", "确认：确认输入和修改"},
};

//unit
static struct labelTransDef unitTrans[] =
{
  {"unit: GHz/dBm/s", "参数设置时相关单位键(GHz/dBm/s)"},
  {"unit: MHz/dB/ms", "参数设置时相关单位键(MHz/dB/ms)"},
  {"unit: kHz/dBmV/us", "参数设置时相关单位键(kHz/dBmV/us)"},
  {"unit: Hz/mV/ns", "参数设置时相关单位键(Hz/mV/ns)"},
};

//number
static struct labelTransDef numberTrans[] =
{
  {"number 0", "数字键0"},
  {"number 1", "数字键1"},
  {"number 2", "数字键2"},
  {"number 3", "数字键3"},
  {"number 4", "数字键4"},
  {"number 5", "数字键5"},
  {"number 6", "数字键6"},
  {"number 7", "数字键7"},
  {"number 8", "数字键8"},
  {"number 9", "数字键9"},
  {"dot", "小数点"},
  {"invert", "符号键，+/-循环切换"},
};

//系统标签汉化
static struct labelTransDef sysTrans[] =
{

  {"Frequency Status Information", "频率状态信息"},
  {"Amplitude Status Information", "幅度状态信息"},
  {"Freq Ref State:\tOn","频率参考状态：开"},
  {"Freq Ref State:\tOff","频率参考状态：关"},
  {"Ref 10MHz:\tInternal","参考时基：内部"},
  {"Ref 10MHz:\tExternal","参考时基：外部"},
  {"Ampl Ref State:\tOn","幅度参考状态：开"},
  {"Ampl Ref State:\tOff", "幅度参考状态：关"},
  { "Ampl ALC State:\tOn","稳幅状态：开" },
  { "Ampl ALC State:\tOff","稳幅状态：关" },
  { "Ampl ALC Response:\tFast","稳幅响应：快" },
  { "Ampl ALC Response:\tSlow","稳幅响应：慢" },
  {"Freq Incr", "频率增量"},
  {"Ampl Incr", "幅度增量"},
  { "Sweep/List Status Information","扫描/列表状态信息" },
  { "Off","关闭" },
  { "On","开启" },
  { "Freq","频  率" },
  { "Ampl","幅  度" },
  { "Amptd", "幅度" },
  { "Fast", "快" },
  { "Slow", "慢" },
  { "Freq&Ampl","频率和幅度" },
  { "Sweep", "扫  描" },
  { "Sweep:\t","扫描模式：" },
  { "Type:\vStep","扫描类型：步进" },
  { "Step", "步进" },
  { "List","列表" },
  { "Type:\vList","扫描类型：列表" },
  { "Freq Start:","起始频率：" },
  { "Freq Stop:","终止频率：" },
  { "Ampl Start:","起始幅度：" },
  { "Ampl Stop:","终止幅度：" },
  { "Retrace:\vOn","扫描折返：开" },
  { "Retrace:\vOff","扫描折返：关" },
  { "Repeat:\vSingle","重复扫描：单次" },
  { "Single","单次" },
  { "Repeat:\vContinuous","重复扫描：连续" },
  { "Cont","连续" },
  { "Points", "点数" },
  { "Direction:\vUp","扫描方向：正向" },
  { "Up","正向" },
  { "Down","反向" },
  { "Direction:\vDown","扫描方向：反向" },
  { "Dwell","驻留时间" },
  { "Trigger:\vAuto","扫描触发：自动扫描" },
  { "Trigger:\vstExt","扫描触发：外部触发" },
  {"Trigger:\vstKey", "扫描触发：按键触发"},
  { "List Mode Values","列表模式值" },
  { "Num","序号" },
  { "Frequency","频率" },
  { "Data File Information","数据文件信息" },
  { "AM Status Information" , "调幅状态信息" },
  { "FM/φM", "调频/调相" },
  { "Modulate Mode:\tFM","调制方式：调频" },
  { "Modulate Mode:\tφM","调制方式：调相" },
  { "FM Status Information", "调频状态信息" },
  { "φM Status Information", "调相状态信息" },
  { "Freq Offset", "频率偏移" },
  { "Phase Offset", "相位偏移" },
  { "FM Dev Couple", "频偏耦合" },
  { "φM Dev Couple", "相偏耦合" },
  { "FM State", "调频状态" },
  { "φM State", "调相状态" },
  { "Analog Modelation Information","模拟调制信息" },
  { "Modulate","调制类型" },
  { "State:\tOn","调制状态：开" },
  { "State:\tOff","调制状态：关" },
  { "FM State:\tOn" , "调频状态：开" },
  { "FM State:\tOff", "调频状态：关" },
  { "φM State:\tOn","调相状态：开" },
  { "φM State:\tOff","调相状态：关" },
  { "Moudulate Depth","调制深度" },
  { "Rate/Width","调制率" },
  { "AM","调  幅" },
  { "FM","调频" },
  { "φM","调相" },
  {"IQ Information", "IQ调制状态信息"},
  {"No Function", "无可用功能"},
  { "Digital Mod","数字调制" },
  { "Digital Modelation Information","数字调制信息" },
  { "Modelation Status:\tOn","调制状态：开" },
  { "Modelation Status:\tOff","调制状态：关" },
  { "Modulation Source", "调制源" },
  { "Digital Source", "调制源" },
  { "Modelation Type","调制类型" },
  { "Deep Coupling" , "深度耦合" },
  { "Depth","深度" },
  { "Devitation","频偏" },
  { "Phase","相偏" },
  { "Rate","速率" },
  { "Pulse Mod", "脉冲调制" },
  { "Pulse Modelation Information","脉冲调制信息" },
  { "Modelation Method","触发方式" },
  { "Pulse Source","脉冲源" },
  { "Pulse Period","脉冲周期" },
  { "Pulse Width","脉冲宽度" },
  { "Free Run","自由运行" },
  { "Key Trig","按键触发" },
  { "Ext Trig","外部触发" },
  { "Pulse Delay" ,"脉冲延时" },
  { "Pulse Polarity","外部脉冲极性" },
  { "Rasing", "上升沿" },
  { "Fall","下降沿" },
  {"LF Out","低频输出"},
  { "Arbitrary Wave Status Information","低频输出信息" },
  { "LFOut Status","输出状态" },
  { "Wave","波形" },
  { "Voltage","电压" },
  { "Sine","正弦波" },
  { "Square","方波" },
  { "Triangle","三角波" },
  { "Sawtooth","锯齿波" },
  { "Sinc","辛格函数" },
  { "Ext Rf", "上变频" },
  {"Exf Ref Information","上变频信息"},
  {"IF Source :\tInternal","变频源：内部"},
  {"IF Source :\tExternal","变频源：外部"},
  {"Ext IF Freq","外部频率"},
  {"Ext IF Ampl", "外部输入信号幅度"},
  {"DRG", "线性调频"},
  {"DRG Information", "线性调频状态信息"},
  {"Up Limit", "扫描上限"},
  {"Down Limit","扫描下限"},
  {"Direction","扫描方向"},
  {"Work Mode", "工作模式"},
  {"Normal","正常"},
  {"Bothway", "双向扫描"},
  {"Positive","正向单路"},
  {"Negative","负向单路"},
  {"USB Power", "功率计"},
  { "USB Power Information", "功率计信息" },
  {"Network Status Information","网络状态信息"},
  {"Channel1",	"通道1"},
  {"Channel2",	"通道2"},
  {"Trigger",	"触发方式"},
  {"State", "状态"},
  {"Pow Offset", "功率偏移"},
  {"Average Mode","平均模式"},
  { "Average Count","平均次数" },
  {"Meas Unit", "测量单位"},
  {"Store",		"存储恢复"},
  {"sysData.Coupling",	"耦合方式"},
  {"OutA",		"OutA"},
  {"OutB",		"OutB"},
  {"OutC",		"OutC"},
  {"OutD",		"OutD"},
  //{"Preset",	"复位"},
  {"Help",		"帮助"},
  {"Man",		"手动触发"},
  {"System",	"系统"},
  {"CLK", 		"时钟"},
  {"CLK1:", 	" 频率:"},
  {"CLK2:", 	" 幅度:"},
  {"Ampt:",		" 电平:"},
  {"DR",		"占空比"},
  {"Duty Ratio:"," 占空比:"},
  {"Delay",		"延时"},
  {"Delay:",	" 延时:"},
  {"PORT",		"端口"},
  {"Level",		"电平"},

  {"Trigger:",	" 触发: "},
  {"  TD:",		" 触发延时："},
  {"CTO",		"连续输出"},
  {"MTO",		"手动触发"},
  {"OTOO",		"单触单出"},
  {"OTCO",		"单触连出"},
  {"Coupling",	"耦合"},
  {"Coupling:",	" 耦合: "},
  {"YES",		"是"},
  {"NO",		"否"},
  {"Ref:",		" 参考: "},
  {"INT",	"内部"},
  {"EXT",	"外部"},
  {"IP:",		" IP地址:"},
  {"Mask:",		" 子网掩码:"},
  {"Gateway:",	" 网关:"},
  {"Level:",	" 电平:"},
  {"FREQ:", 		" 频率:"},
  {"AMPTD:", 	" 幅度:"},

  {"Frequency Range: 9kHz - 3GHz",						"频率范围：9kHz - 3GHz"},
  {"Amplitude Range: -130dBm - +20dBm",					"幅度范围：-130dBm - +20dBm"},

  {"reseting system......", "复位系统......"},
  {"Ready",              "就绪"},
  {"CLK1 Frequency Range: 1Hz - 100KHz",					"时钟1频率范围：1Hz - 100KHz"},
  {"CLK2 Frequency Range: 1Hz - 100KHz",					"时钟2频率范围：1Hz - 100KHz"},
  {"Channel1 Amplitude Options: 1.5V 1.8V 2.5V 3.3V",		"通道1电平选项：1.5V 1.8V 2.5V 3.3V"},
  {"Channel2 Amplitude Options: 1.5V 1.8V 2.5V 3.3V",		"通道2电平选项：1.5V 1.8V 2.5V 3.3V"},
  {"Channel1 Delay Range: 0ns - 1s (5ns step)",				"通道1延时范围：0ns - 1s (5ns 步进)"},
  {"Channel2 Delay Range: 0ns - 1s (5ns step)",				"通道2延时范围：0ns - 1s (5ns 步进)"},
  {"Channel1 Duty Ratio Range:0.1% - 99.0%",				"通道1占空比范围：0.1% - 99.0%"},
  {"Channel2 Duty Ratio Range:0.1% - 99.0%",				"通道2占空比范围：0.1% - 99.0%"},
  {"Continuous Trigger Output",								"连续触发输出"},
  {"Manual Trigger Output",									"手动触发输出"},
  {"External Signal Once Trigger Once Output",				"外部信号单次触发后单次输出"},
  {"External Signal Once Trigger Continuation Output",		"外部信号单次触发后连续输出"},
  {"Internal Reference Clock",								"内部参考时钟"},
  {"External Reference Clock",								"外部参考时钟"},
  {"Trigger Delay Range: 50us - 1s (5ns step)",				"触发延时范围：50us - 1s (5ns 步进)"},

  {"IP Address Set",										"IP地址设置"},
  {"Mask Set",												"子网掩码设置"},
  {"Gateway Address Set",									"网关地址设置"},

//  {"Channel1 coupled to the CLK1 & Channel2 coupled to the CLK2",					"通道1耦合至时钟1，通道2耦合至时钟2"},
//  {"Channel1 coupled to the CLK2 & Channel2 coupled to the CLK1",					"通道1耦合至时钟2，通道2耦合至时钟1"},

  {"Channel1 coupled to the Clock2",					"通道1耦合至时钟2"},
  {"Channel1 coupled to the Clock1",					"通道1耦合至时钟1"},
  {"Channel2 coupled to the Clock1",					"通道2耦合至时钟1"},
  {"Channel2 coupled to the Clock2",					"通道2耦合至时钟2"},

  {"switch to local control mode", "恢复为本地控制模式"},
  {"switch to remote control mode", "切换为远程控制模式"},
  {"remote control mode, pressed key is ignored", "远程控制模式，本地按键忽略"},

  {"Press any key for show help content", "按任意键显示帮助内容"},
  {"pressed help key again to close", "再次按下帮助键关闭"},

  {"Name", "名称"},
  {"Time", "时间"},
  {"Size", "大小"},
  {"Size(Bytes)", "大小(字节)"},
  {"DateTime", "日期/时间"},

  {"file delete success", "文件删除成功"},
  {"file delete failed", "文件删除失败"},
  {"file export success", "文件导出成功"},
  {"file export failed", "文件导出失败"},

  {"Total Files", "文件总数"},
//  {"Screen Files", "图片文件"},
//  {"Trace Files", "迹线数据文件"},

  {"Default", "默认"},
  {"Please input RegCode", "请输入注册码"},
  {"Register Success", "注册成功"},
  {"Register Failed", "注册失败"},

  {"Updating System", "正在更新系统"},
  {"Updating File", "更新文件"},
  {"Updating Success", "更新完成"},
  {"Rebooting", "重新启动"},
  {"Update success to", "系统成功更新至"},
  {"Old Version is", "原版本为"},

  {"saving user state", "正在存储用户状态"},
  {"user state saved", "用户状态储存完成"},

  {"Detecting usb disk", "搜索USB设备"},
  {"Saving file to usb disk", "写入USB设备"},

//  {"fd module communication error", "鉴频模块通讯故障"},
  {"system auto reseting", "系统自动复位"},

//  {"signal searching", "搜索信号"},
  {"device connecting", "连接设备"},
  {"export to usb disk failed", "输出至USB闪存失败"},

  {"date set successed", "日期设置成功"},
  {"date set failed", "日期设置失败"},
  {"time set successed", "时间设置成功"},
  {"time set failed", "时间设置失败"},

  {"Load data file...", "装载数据文件..."},
  {"Load data file failed","装载失败"},
  {"Device Type", "设备型号"},
  {"Do Not Support The Function","不支持该功能"},
  {"Power Connected Failed","功率计连接失败"},
};


/********************** U盘目录相关 ***************************/
#define LISTDATAENTRY		0xFFFF0000		//列表扫描数据文件
#define DIGITALDATAENTRY	0xFFFF0001
//U盘目录结构
struct UsbStorageEntry
{
	unsigned long num;
	char folder[64];
};
static struct UsbStorageEntry Upan[] = {
		{LISTDATAENTRY, "ListData/"},
		{DIGITALDATAENTRY, "DigitalData"},
};


//LAN结构
struct lanDef
{
  char ip[64];
  char mask[64];
  char gate[64];
  char mac[64];
  unsigned long macLo;
};

//COM结构
struct comDef
{
  baudRate baudrate;
  dataBits databits;
  stopBits stopbits;
  checkMode check;
  flowControl flow;
};

//系统I/O结构定义
struct ioDef
{
  lanDef lan;   //264
  comDef com;  //20
};


//系统结构定义
struct systemDef
{
  bool isLanguageEn;
  bool isInternalSignalOn;
  //bool isScpiScreenRefresh;
  char serialNo[32];
  int lanMacAddrIndex;
  unsigned char lanMacAddress[12];
  int backLight;
  bool isScpiFresh;
  bool isServiceUnlock;                       //系统服务是否开启

  ioDef io;        //294 +58
};


//SCPI通信数据结构定义
struct scpiCommDataDef
{
  scpiInterfaceType interface;

  bool stateR;
  bool stateL;
  bool stateT;
  bool stateS;

  bool serialOpened;
  bool lanOpened;
  bool usbOpened;

  unsigned char buf[SCPIBUFSIZE];
  unsigned int rcvLength;
};

struct Delay
{
	char ValueChar[64];
	double Value;
	char UnitChar[64];			//单位，字符串类型
};

struct PG100_CommonPara
{
	char NameChar[64];
	char ValueChar[64];
};

struct listDataDef
{
  double freq;
  float ampl;
  float dwell;
};

struct amParaDef
{
  bool state;
  double depth;							//深度
  amInternalWaveTypeDef waveType;		//波形
  double rate;							//调制率
  bool source;							//调制源 true : 外部， false: 内部
  bool coupling;						//深度耦合
};

struct fmParaDef
{
  bool state;						//状态
  double deviation;                 //频偏
  fmInternalWaveTypeDef waveType;	//波形
  bool source;						//调制源 true : 外部， false: 内部
  double rate;                      //调制率
  bool coupling;					//频偏耦合
};

struct pmParaDef
{
  bool state;						//状态
  double phase;                     //相位
  pmInternalWaveTypeDef waveType;		//波形
  bool source;						//调制源 true : 外部， false: 内部
  double rate;                      //调制率
  bool coupling;					//频偏耦合
};


struct askParaDef
{
  bool state;
  askModeDef mode;
  double rate;
};

struct fskParaDef
{
  bool state;
  fskModeDef mode;
  double rate;
  double step;
};

struct pskParaDef
{
  bool state;
  pskModeDef mode;
  double rate;
};

struct digitalPara                      //数字调制SG10301A
{
	double depth;                       //深度
	double deviation;                   //频偏
	double phase;                       //相偏
	double rate;                        //调制率
};

struct exfRefDef	//上变频
{
  extRefFrequencyTypeDef type;            //类型
  double freq;                            //频率
};


struct refDef
{
  double value;
  bool state;
};

struct alcDdf  //ALC结构体
{
  bool state;//开关
  bool response;//快慢
};

struct freqDef        //49字节
{
  double freq;
  double maxValue;
  double minValue;
  refDef ref;

  double prevValue;
  double downLoad;
  double offset;
  int multiplier;
  freqRefTypeDef refType;

  ref10MHzDef ref10MHz;

};

struct errorDef
{
  bool isNeeded;
  double value;
};

struct amplDef          //41字节
{
  double ampl;
  double amplInit;         	//原始值
  amplUnitDef amplUnit;    	//Ampl的下发值的单位
//  amplUnitDef amplinitUnit;	//显示输入值的单位，用于在屏幕第三部分显示
  refDef ref;
  double setValue;		//用户设置的幅度值   zpc add
  double dispValue;		//显示值
  double prevValue;
  double downLoad;
  double offset;
  errorDef error;         //差损

  alcDdf alc;
};

struct sweepStepDef
{
  double dwell;
  double freqStart;
  double freqStop;
  double amplStart;
  double amplStop;
  int point;
};

struct sweepListDef   //70 +512*16
{
  char nameChar[64];
  bool enable;
  listDataDef data[512];

  double maxValue;
  double minValue;
  int length;
  int currentPage;
  int indexInPage;
  int allIndex;
  int allRow;			//所有行数
  int row;
  char unitChar[8];

  int displayPage;
  int allPage;

  int selectRow;
  int selectLine;

  bool selectDisplay;
  bool download;

  float ratio;

};

struct sweepDef
{
  unsigned short nowpagenum;      //列表扫描中参数区当前页数
//  unsigned short totalpagenum;    //列表扫描中参数区总页数

  sweepModeDef mode;   //4
//  bool state;		//总开关
//  bool freqstate;	//频率开关
//  bool amplstate;	//幅度开关
  sweepTypeDef type; //4
//  int type;

  sweepRepeatDef repeat;
  sweepRetraceDef retrace;
  sweepDirectionDef direction;   //16
  sweepTriggerDef trig;
  char trigger[32];              //32

  sweepStepDef step;             //44

  sweepListDef list;
};

//FM/PM调制
struct fmpmModulateDef
{
	fmpmDemodTypeDef mode;
	fmParaDef fm;
	pmParaDef pm;
};

//数字调制
struct digitalModulateDef
{
  //digitalDemodTypeDef type;
  //askParaDef ask;
  //fskParaDef fsk;
  //pskParaDef psk;
  digitalTypeDef digitaltype;    //SG10301A和SG10301B分别进行了定义
  bool digitalstate;             //数字调制状态
  digitalPara digitalpara;                   //调制参数
  digitalSourceTypeDef source;

  unsigned char sequence[3079];		//数字调制的外部调制序列
  unsigned int sequenceCntBefore;	//序列转为unsigned char前的个数
  unsigned int sequenceCnt;			//序列转为unsigned char后的个数
};

//脉冲调制
struct pulseParaDef
{
  bool state;						//状态
  pulseMethodDef trigmethod;		//脉冲触发方式
  pulseSourceDef pulsesource;		//脉冲源
  double period;					//周期，单位ns
  double width;						//宽度，单位ns
  double delay;						//脉冲延时，单位ns
  pulseExtPolarityDef polarity;		//外部触发信号极性
};



//LF
struct lFParaDef
{
  lfOutputTypeDef type;
  double freq;
  double ampl;	//单位 mv
  bool state;
};


//DRG
struct drgParaDef
{
	bool state;					//DRG状态
	double uplimit;				//DRG扫描上限
	double downlimit;			//DRG扫描下限
	int	points;					//DRG扫描点数
	double rate;				//DRG扫描速率，单位 ns
	drgDireTypeDef workmode;	//DRG工作模式
};
//USB Power
struct usbPowerParaDef
{
	bool state;						//USB功率计状态
	double freq;					//通道频率
	double powerOffset;				//功率偏移
	usbPowerAdvModeTypeDef AdvMode;	//平均模式
	double AdvCount;				//平均次数
	usbPowerMeasUnitTypeDef MeasUnit;//测量单位
	double rcvValue;					//从功率计中接收到幅度字符串
	QList<double> list;					//存放从功率计中接收到数据
};


//More key
struct menuMoreCountDef
{
	int Count;			//当前菜单一共多少个More键，从0开始
	int index;				//当前是第几个More键， 从0开始
};

//IncrSet key
struct incrsetParaDef
{
	incrsetTypeDef type;	//当前增量类型
	double FreqIncrValue;	//频率增量
	double AmptdIncrValue;	//幅度增量
};

struct LEDParaDef
{
	bool Power;	//电源指示灯
	bool More;	//More键指示灯
	bool LFOut;	//LF指示灯
	bool Mod;	//Mod指示灯
	bool RF1;	//RF1指示灯
	bool RF2;	//RF2指示灯
};

//系统数据结构定义
struct dataDef
{
  char name[64];
  char value[128];
  char amplinitvalue[128];  //幅度中显示用户输入的参数
  char unit[8];
  int inputLength;
  drawTypeDef drawType;

  bool cursorDirection;
  int cursorLocation;     //光标位置

  double rpgReturnValue;

  LEDParaDef led;	//面板LED指示灯

  freqDef freq;                                           //49
  amplDef ampl;                                           //41
  sweepDef sweep;                                         //8262
  incrsetParaDef incrSet;

  fmpmModulateDef fmpm;                              //68  

  amParaDef am;

  digitalModulateDef digital;                            //68

  pulseParaDef pulse;                                     //脉冲调制

  lFParaDef	lf;                                          //21

  drgParaDef drg;

  usbPowerParaDef usbPower;							//USB功率计

  bool isNeedCable;

  //bool keyBoardSwitchMod,keyBoardSwitchLF,keyBoardSwitchRF;

  char remoteConnection;		                         //5

  struct scpiCommDataDef scpiData;                       //SCPI信息    8192 +16  8208

  exfRefDef exfRef;                                     //上变频

  int indexPoints;
  int process;
  bool isBeginDrawSingleProcess;

  menuMoreCountDef moreKeyCount;

  systemDef system;                                      //352
  char titleName[64];	//菜单标题
};

//系统数据结构定义
struct powerOffDataDef
{
  char name[64];
  char value[128];
  char unit[8];
  int inputLength;
  drawTypeDef drawType;

  bool cursorDirection;
  int cursorLocation;

  double rpgReturnValue;

  freqDef freq;                                           //49
  amplDef ampl;                                           //41
  sweepDef sweep;                                         //8262

  fmpmModulateDef fmpm;                              //68

  digitalModulateDef digital;                            //68

  lFParaDef	lf;                                          //21

  bool isNeedCable;

  bool keyBoardSwitchMod,keyBoardSwitchLF,keyBoardSwitchRF;

  char remoteConnection;		                         //5

  struct scpiCommDataDef scpiData;                       //SCPI信息    8192 +16  8208

  exfRefDef exfRef;                                     //上变频
  int indexPoints;
  int process;
  bool isBeginDrawSingleProcess;
};


//子键定义
struct subKeyDef
{
  char* tagName;                        //标签

  char* keyValueStr;                    //键值名称
  char* funcCodeStr;                    //功能码名称

  int keyValue;                         //键值
  int funcCode;                         //功能码
  int keyTimes;                         //按键次数

  char* enName;                         //英文名称
  char* chName;                         //中文名称
  char* enHelp;                         //英文帮助
  char* chHelp;                         //中文帮助

  bool isEnabled;                       //使能
  bool isBlank;                         //是否为空白键
  bool isInput;                         //是否为输入
  bool isReverse;                       //是否为反转
  bool isList;                          //是否为列表
  bool isUnit;                          //是否为单位键
  bool isExecute;                       //是否为执行键
  bool isMore;                          //是否直接包含子菜单
  bool isReturn;                        //是否为返回键
  bool isPassword;                      //是否为口令
  bool isDate;                          //是否为日期
  bool isTime;                          //是否为时间
  bool isDateTime;                      //是否为日期时间
  bool isIP;                            //是否为IP地址
  bool isMac;                           //是否为Mac地址

  struct subKeyDef* upNode;             //上节点
  struct subKeyDef* downNode;           //下节点
  struct subKeyDef* leftNode;           //父节点
  struct subKeyDef* rightNode;          //子节点
};

//主键定义
struct mainKeyDef
{
  char* tagName;                        //标签

  char* keyValueStr;                    //键值名称
  char* funcCodeStr;                    //功能码名称

  int keyValue;                         //键值
  int funcCode;                         //功能码

  char* enName;                         //英文名称
  char* chName;                         //中文名称
  char* enHelp;                         //英文帮助
  char* chHelp;                         //中文帮助

  struct subKeyDef* rightNode;          //主键内子键
  struct mainKeyDef* downNode;          //下一个主键
};


#endif
