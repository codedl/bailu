...
static bool zerospan;//zerospan work status 
static bool data_irq;//flag of reading data in zerospan
...
{	
	 if (fasync_queue && zerospan == true && data_irq == true)		
	 {
...
{
	init_timer(&timer);
	timer.expires = jiffies + 20;//定时器到期时间
	timer.data = 5;//传递给定时器函数的参数
...
static int bl_ram_read(struct file *filp, char __user *buff, size_t count, loff_t *offp)
{
	...
	memset(&ramData[0],0,sizeof (ramData));
	...
	for(i = 0; i < RAMBUFSIZE; i++)
	{
		...
		iowrite8(1, remapBuf + 38 * 2);wmb();
		iowrite8(0, remapBuf + 38 * 2);wmb();
		ndelay(5);
