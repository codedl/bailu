...
static unsigned int FpgaData[datasize];//保存fpga读到的数据
...
static long bl_fe_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd)
	{
		...
		case 0xfffa:
			...
			break;
		case 0xfffc://fpga软件版本号
			FpgaData[0] = ioread16(remapBuf + 0 * 2);rmb();    //高8位
			FpgaData[1] = ioread16(remapBuf + 1 * 2);rmb();    //低16位
			break;
		case 0xfffb:
