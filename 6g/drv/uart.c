...
#define DEVICE_NAME 	"BL_OMAPL138_UART"
...
#endif
	/* 配置复选引脚 */
	void __iomem *   conf_pinmux4_remapbuf = ioremap(0x01C14130 , 4);
	void __iomem *   conf_uart_pow_remapbuf = ioremap(0x01D0C030, 4);
	
	regvalue = ioread32(conf_pinmux4_remapbuf); rmb();
	regvalue |= (0x22 << 24);
	iowrite32(regvalue, conf_pinmux4_remapbuf);wmb();
	iounmap(conf_pinmux4_remapbuf);
	
	/*配置电源管理*/
	regpowvalue = ioread32(conf_uart_pow_remapbuf); rmb();
	regpowvalue |= ((1) | (0x3 << 13));
	iowrite32(regpowvalue, conf_uart_pow_remapbuf);wmb();
	iounmap(conf_uart_pow_remapbuf);
}
...
MODULE_DESCRIPTION("module driver of uart1");
