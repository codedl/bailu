...
//    void checkMultiDisplay(multiDispType value);
  ...
  signals:
     void showAlarmMessage(QString title, QString msg);
     void refreshScpiState(QString helpStr);                      //刷新SCPI状态
     void refreshWindowState(void);                               //刷新窗口状态
     void resetUsbSlaveInterface(void);                           //复位USB从口

     void drawProcess(void);                                           //绘制 process
     void drawsListData(void);                                         //绘制列表扫描编辑
     void drawProgBar(bool Display, float RatioProgBar);               //绘制进度条
     void drawDownloadProg(bool Display, int current, int amount);     //绘制加载进度
     void drawAnalogModulate(void);                                    //绘制模拟调制
     void drawDigitalModulate(void);                                   //绘制数字调制
	 void drawPluseModulate(void);                                    //绘制脉冲调制
	 void drawSystemPara(void);                                        //绘制系统参数
     void drawArbitraryWave(void);                                     //绘制任意波
//    virtual void drawFreqAndAmpl(void);                                       //绘制频率幅度
	 void drawFreqInParamter(void);
	 void drawAmplInParamter(void);
     void drawExfFref(void);                                           //绘制参考
     void drawDrgFunction();											  //绘制DRG
     void drawSweep(int type);                                         //绘制扫描

     void drawFreqAndAmplMain(void);		                              //绘制主参数显示区
     void drawState(void);	   	                                  //绘制背景
     void drawInputParamterNoParamter(void);	                              //绘制输入栏显示区
     void drawInputParamter(char* str, int funcode);                   //绘制输入栏显示区
//    virtual void drawHelp(void);                                              //绘制帮助栏显示区
     void drawButton(void);                                            //绘制按钮显示区
