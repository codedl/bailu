...
int tSysScpi::setFrequencyOfStart(double value)
{
	...
	zeroSpanHandle();
	controlRf();
	qDebug() << "sysData.freq.start:" << sysData.freq.start;
	return __SCPI_SUCCESS;
}
...
int tSysScpi::setAmptOfAtt(double value)
{
	...
	if(sysData.freq.stop <= 20e6){//终止频率小于20M时通过DC板控制
		controlDc();
		return __SCPI_SUCCESS;
	}
...
{
	if(sysData.freq.stop <= 20e6)//通过DC板控制
	{
		controlDc();
		return;
	}
	
	feDownload(ADDR_CONA,1);
#if 0
	bwAutoCouple();
...
double tSysScpi::getFreqValue(QString value, bool* ok)
{
	*ok = false;
	...
	for (int i = 0; i < unitList.count(); i++)
		if (valueUpper.indexOf(unitList.at(i)) != -1)//判断单位
		{
			valueUpper = valueUpper.remove(unitList.at(i)).trimmed();//去掉单位
			tempValue = valueUpper.toDouble(&isOk);//字符串中的数字部分

			if (isOk)
			{
				switch (i)//转化成以HZ为单位的数字
				{
					case 0:
						tempValue *= 1e9;//GHZ
						break;
...
//计算控制DC前放,衰减送数
int tSysScpi::getDcPremptData()
{
	int ret = 0;
	if(sysData.ampt.isPreamptOn)
		ret |= 0x02;//前置放大打开
	if(sysData.freq.stop < 150e3)//预滤波通道
		ret |= 0x04;
	else
		ret |= 0x08;
	ret |= 0x30;
	if(sysData.ampt.att >= 21)
		ret &= 0xCF;
	else if(sysData.ampt.att >= 10)
		ret &= 0xEF;
	return ret;
}
...
int tSysScpi::setLowbandAtt(double value)
{
	...
	sysData.ampt.attRf = value;
	int attrf = (int)value << 2;//低两位清零
	int temp = (0x03 << 8 ) | (attrf & 0xff);//高8位固定为0x03
